...
@interface XianJinQuanVC ()

@end

@implementation XianJinQuanVC
{
...
- (void)viewDidLoad
{
    ...
    [self initUI];
    unCurrPage = 1;
    [self request:0 page:unCurrPage cardNum:nil psd:nil];
}
...
- (void)initUI
{
    ...
    segment.segments = [NSArray arrayWithObjects:@"可使用",@"绑定", nil];
    ...
    UILabel *aLable = [[UILabel alloc]initWithFrame:CGRectMake(12, 0, 280, 25)];
    aLable.hidden = NO;
    [aLable setText:@"提示：一次可使用多张现金券"];
    [aLable setTextColor:TEXT_GRAY_COLOR];
    aLable.font = SYSTEMFONT(12);
    aLable.backgroundColor = [UIColor clearColor];
    unUsedTableView.tableHeaderView = aLable;
    [aLable release];
...
- (void)request:(int)type page:(int)page cardNum:(NSString *)cardNum psd:(NSString *)psd     // type: 0 获取列表 1 绑定      subtype: 1 已使用  2 未使用
{
    ...
    if (type == 0) {
        [request requestDataWithInterface:GetUseableShoppingTicket param:[self GetUseableShoppingTicketParam:1 page:page buytype:self.orderType] tag:1];
    }else {
        [request requestDataWithInterface:BindShoppingTicket param:[self BindShoppingTicketParam:cardNum password:psd type:1 haspassword:1 isscan:0]tag:100];
    }
    ...
    switch (index) {
        case 0:
        {
            ...
            if (strNum.length > 0 && strPsd.length > 0) {
                [self request:1 page:0 cardNum:strNum psd:strPsd];
            }else {
...
- (void)segmentControl:(MySegMentControl *)setMentControl touchedAtIndex:(NSInteger)index
{
    ...
    switch (index) {
        case 0:
            ...
            break;
        case 1:
            if (!bdView) {//  bug：  切换时键盘没有隐藏
                bdView = [[BangdingView alloc]initWithFrame:CGRectMake(MainViewWidth * 1, 0, sv.frame.size.width , sv.frame.size.height)];
                bdView.ifHasPsd = 2;
                [bdView initUI];
...
-(void)leftAction
{
    ...
    for (ShoppingTicketEntity *entity in arrUnUsed) {
        if (entity.isSelected) {
            [myArray addObject:entity];
        }
    }
    
//    if ([self.delegate performSelector:@selector(sendFormMessage:Object:)]&& [arrUnUsed count]!=0) {
//        [self.delegate sendFormMessage:InfoTypeXianjin Object:myArray];
//    }
    [self.delegate sendFormMessage:InfoTypeXianjin Object:myArray];
