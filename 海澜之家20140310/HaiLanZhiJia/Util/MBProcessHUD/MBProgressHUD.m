//版权所有：版权所有(C) 2011，东信移动控股有限公司
//系统名称：无线研发中心
//文件名称：HouseShowViewController
//作　　者：郭勇
//完成日期：2012-6-13
//功能说明：室内360全景视图控制器
//-----------------------------------------
...
- (void)layoutSubviews {
    ...
    if (nil != self.labelText) {
        ...
        float lWidth;
        if (dims.width <= (frame.size.width - 3 * margin)) {
            lWidth = dims.width;
        }
        else {
            lWidth = frame.size.width - 3 * margin;
        }
        ...
        CGRect lFrame = CGRectMake(floorf((frame.size.width - lWidth) / 2) + xOffset-margin,
                                   floorf(indFrame.origin.y + indFrame.size.height + PADDING),
                                   lWidth+2*margin, lHeight);
